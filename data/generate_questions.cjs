...
const text = `O valor máximo da contagem na saída do Timer0 com prescaler de 1:64 é: A) 256 eventos B) 1024 eventos C) 8192 eventos D) 16384 eventos (Correta) E) 32768 eventos
...
const pattern = /(.*?)\s+A\)\s+(.*?)\s+B\)\s+(.*?)\s+C\)\s+(.*?)\s+D\)\s+(.*?)\s+E\)\s+(.*)/s;

for (let block of blocks) {
    block = block.trim();
    if (!block || block.startsWith('PDF 2:')) continue;
    
    const match = block.match(pattern);
    if (match) {
        const q_text = match[1].trim();
        const options = [
            match[2].trim(),
            match[3].trim(),
            match[4].trim(),
        ...
        let answer = null;
        const cleaned_options = [];
        
        for (let opt of options) {
            if (opt.includes('(Correta')) {
                const clean_opt = opt.replace(/\(Correta.*/, '').trim();
                cleaned_options.push(clean_opt);
                answer = clean_opt;
            } else {
...
const ts_content = `import type { Question } from "../types/question";\n\nexport const microcontroladoresP2Questions: Question[] = ${JSON.stringify(questions, null, 2)};\n`;

fs.writeFileSync('g:/ERICK/src/data/microcontroladores_p2.ts', ts_content, 'utf8');
